...
import types.{
  type Model, type Msg, DisclosureGroupMsg, MenuItemSelected, MenuMsg, NoOp,
  PopoverMsg, TabSelected, ToolbarMsg,
}
    ...
    tailwind.vstack_lg([
        ...
        tabs.tabs([attribute("data-testid", "tabs-demo")], [
          tabs.tab_list(
            [
              ...
              // Add accessible name to tablist (BUG-TB-03)
              attribute("aria-label", "Example Tabs"),
                ...
                case key {
                  ...
                  "ArrowLeft" -> {
                    case model.tabs_selected {
                      ...
                      "disabled" -> TabSelected("details")
                      _ -> TabSelected("overview")
                    }
                  }
                  "Home" -> TabSelected("overview")
                  "End" -> TabSelected("disabled")
                  // Enter/Space handled by individual tabs
            ...
            [
              tabs.tab(
                [
                  ...
                  // Roving tabindex: selected tab is focusable (BUG-TB-07, TB-08)
                  attribute("tabindex", case model.tabs_selected == "overview" {
                    True -> "0"
                    False -> "-1"
                  }),
                  event.on_click(TabSelected("overview")),
                  // Handle Enter/Space on the tab itself
                  event.on_keydown(fn(key) {
                    case key {
                      "Enter" | " " -> TabSelected("overview")
                      _ -> types.NoOp
                    }
                  }),
                ],
              ...
              tabs.tab(
                [
                  ...
                  // Explicitly set aria-selected for disabled tab
                  tabs.tab_selected(model.tabs_selected == "disabled"),
                  attribute("id", "demo-tab-disabled"),
                  attribute("aria-controls", "demo-tabpanel-disabled"),
                  // Disabled tabs should not be focusable
          ...
          // Add tabpanel for disabled tab to satisfy aria-controls reference (BUG-TB-02)
          tabs.tab_panel(
            [
              attribute("id", "demo-tabpanel-disabled"),
              attribute("aria-labelledby", "demo-tab-disabled"),
              class(case model.tabs_selected == "disabled" {
                True -> ""
                False -> " hidden"
              }),
            ],
            [text("Disabled tab panel content.")],
          ),
        ]),
      ]),
      tailwind.field([
        ...
        div(
          [
            ...
            attribute("data-testid", "menu-demo"),
          ],
          [
            menu.trigger(
              [
                    ...
                    [
                      menu.menu_item(
                        [
                          ...
                          attribute(
                            "tabindex",
                            case menu_utils.is_highlighted(model.menu, 0) {
                              True -> "0"
                              False -> "-1"
                            },
                          ),
                          ...
                          event.on_focus(
                            MenuMsg(menu_utils.SetHighlightedIndex(0)),
                          ),
                        ],
                        [text("New file")],
                      ),
                      menu.menu_item(
                        [
                          ...
                          attribute(
                            "tabindex",
                            case menu_utils.is_highlighted(model.menu, 1) {
                              True -> "0"
                              False -> "-1"
                            },
                          ),
                          ...
                          event.on_focus(
                            MenuMsg(menu_utils.SetHighlightedIndex(1)),
                          ),
                        ],
                        [text("Rename")],
                      ),
                      ...
                      menu.menu_section([], "Preferences", [
                        menu.menu_item_checkbox(
                          [
                            ...
                            attribute(
                              "tabindex",
                              case menu_utils.is_highlighted(model.menu, 2) {
                                True -> "0"
                                False -> "-1"
                              },
                            ),
                            ...
                            event.on_focus(
                              MenuMsg(menu_utils.SetHighlightedIndex(2)),
                            ),
                          ],
                          True,
                          [text("Autosave")],
                        ...
                        menu.menu_item_radio(
                          [
                            ...
                            attribute(
                              "tabindex",
                              case menu_utils.is_highlighted(model.menu, 3) {
                                True -> "0"
                                False -> "-1"
                              },
                            ),
                            ...
                            event.on_focus(
                              MenuMsg(menu_utils.SetHighlightedIndex(3)),
                            ),
                          ],
                          True,
                          [text("Compact mode")],
      ...
      tailwind.field([
        ...
        div(
          [
            class("relative inline-block"),
            attribute("data-testid", "popover-section"),
          ],
          [
            popover.trigger(
              [
                ...
                attribute(
                  "aria-expanded",
                  case popover_utils.is_open(model.popover) {
                    True -> "true"
                    False -> "false"
                  },
                ),
    ...
    list.map(indices, fn(index) {
      case toolbar_utils.item_at(model.toolbar, index) {
        Some(item) -> {
          ...
          let is_highlighted =
            toolbar_utils.is_highlighted(model.toolbar, index)
          let variant = case is_highlighted {
            True -> button.Secondary
            False -> button.Ghost
          }
          let aria_pressed = case is_highlighted {
            True -> "true"
            False -> "false"
          }
          button.button(
            [
              ...
              event.on_click(
                ToolbarMsg(toolbar_utils.SetHighlightedIndex(index)),
              ),
            ],
            [text(item.label)],
          )
  ...
  let all_children =
    list.append(button_items, [
      tailwind.helper_text_text(
        "Focus with Tab, then use Arrow keys to navigate. Enter/Space activates the focused button. Home/End jump to first/last.",
      ),
    ])
  tailwind.vstack_md([
...
pub fn view_disclosure_group(model: Model) -> Element(Msg) {
  ...
  let disclosure_items =
    list.map(indices, fn(index) {
      let is_expanded =
        disclosure_group_utils.is_expanded(model.disclosure_group, index)
      let tabindex =
        disclosure_group_utils.tabindex_for(model.disclosure_group, index)
      div([class("border rounded-md")], [
        button.button(
          [
            ...
            attribute(
              "id",
              disclosure_group_utils.disclosure_trigger_element_id(index),
            ),
            attribute("tabindex", int.to_string(tabindex)),
            ...
            attribute(
              "aria-controls",
              disclosure_group_utils.disclosure_content_element_id(index),
            ),
            event.on_click(
              DisclosureGroupMsg(disclosure_group_utils.Toggle(index)),
            ),
          ],
  ...
  let all_children =
    list.append(disclosure_items, [
      tailwind.helper_text_text(
        "Focus with Tab, then use Arrow keys to navigate between headers. Enter/Space expands/collapses. Home/End jump to first/last.",
      ),
    ])
  tailwind.vstack_md([
